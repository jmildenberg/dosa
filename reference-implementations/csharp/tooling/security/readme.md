# C# :: Tooling :: Security

> [C#](../../readme.md) :: [Tooling](../readme.md) :: Security

Local configuration for the pipeline security gates this domain runs in [CI](../ci/readme.md): SAST, SCA
(dependency scanning), container image scanning, and DAST configuration/suppression files, plus any
local pre-merge equivalents wired through [githooks/](../githooks/readme.md). This is configuration
only — the standard the gates enforce is defined once at the org level, not restated here.

---

References: [docs/security.md](../../../../docs/security.md)
