...
**Depends on:** nothing — no Domain internal leaks into a contract type. **Depended on by:** Clients, and
by any other domain that consumes this one. Contracts are the *only* thing another domain is allowed to
depend on — a breaking change here is a breaking change for every consumer, not an internal refactor.
